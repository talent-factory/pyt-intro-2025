...
def test_passwort_validierung(
    ...
    with patch("builtins.input", return_value="q"):
        ...
        if sollte_gueltig_sein:
            assert ist_gueltig, (
                f"Passwort sollte gültig sein: {passwort}, Fehler: {fehler}"
            )
            assert len(fehler) == 0
        else:
...
def test_passwort_grossbuchstabe() -> None:
    ...
    with patch("builtins.input", return_value="q"):
        ...
        assert not ist_gueltig
        assert any(
            "Grossbuchstabe" in str(f) or "grossbuchstabe" in str(f).lower()
            for f in fehler
        )
...
def test_passwort_kleinbuchstabe() -> None:
    ...
    with patch("builtins.input", return_value="q"):
        ...
        assert not ist_gueltig
        assert any(
            "Kleinbuchstabe" in str(f) or "kleinbuchstabe" in str(f).lower()
            for f in fehler
        )
...
def test_passwort_sonderzeichen() -> None:
    ...
    with patch("builtins.input", return_value="q"):
        ...
        assert not ist_gueltig
        assert any(
            "Sonderzeichen" in str(f) or "sonderzeichen" in str(f).lower()
            for f in fehler
        )
